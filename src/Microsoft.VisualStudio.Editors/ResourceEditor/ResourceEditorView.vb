...
#Region "UI Initialization"
        ...
        Private Sub SetFonts(ServiceProvider As IServiceProvider)
            ...
            StringTableFont = GetFontFromResources(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Font_StringTable)
            ListViewFont = GetFontFromResources(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Font_ListView)
        ...
        Private Sub RegisterMenuCommands(isEditingResWFile As Boolean)
            ...
            Try
                ...
                Else
                    ...
                    _viewsLatchedCommandGroup.Add(ResourceListView.ResourceView.List, New DesignerMenuCommand(RootDesigner, Constants.MenuConstants.CommandIDRESXViewsList, AddressOf ButtonViews_List_Click, AddressOf ViewsMenuItemsEnabledHandler, AlwaysCheckStatus:=True))
                    _viewsLatchedCommandGroup.Add(ResourceListView.ResourceView.Details, New DesignerMenuCommand(RootDesigner, Constants.MenuConstants.CommandIDRESXViewsDetails, AddressOf ButtonViews_Details_Click, AddressOf ViewsMenuItemsEnabledHandler, AlwaysCheckStatus:=True))
                    _viewsLatchedCommandGroup.Add(ResourceListView.ResourceView.Thumbnail, New DesignerMenuCommand(RootDesigner, Constants.MenuConstants.CommandIDRESXViewsThumbnails, AddressOf ButtonViews_Thumbnail_Click, AddressOf ViewsMenuItemsEnabledHandler, AlwaysCheckStatus:=True))
                    For Each Command As MenuCommand In _viewsLatchedCommandGroup.Commands
                        _menuCommands.Add(Command)
                    Next

                    'Import/Export
                    _menuCommands.Add(New DesignerMenuCommand(RootDesigner, Constants.MenuConstants.CommandIDResXImport, AddressOf MenuImport, AddressOf MenuImportEnabledHandler,
                        AlwaysCheckStatus:=True))
                    _menuCommands.Add(New DesignerMenuCommand(RootDesigner, Constants.MenuConstants.CommandIDResXExport, AddressOf MenuExport, AddressOf MenuExportEnabledHandler,
                        AlwaysCheckStatus:=True))
                ...
                'Delete
                '
                _menuCommands.Add(New DesignerMenuCommand(RootDesigner, Constants.MenuConstants.CommandIDVSStd97cmdidDelete, AddressOf MenuRemove, AddressOf MenuDeleteEnabledHandler,
                    CommandVisibleHandler:=AddressOf MenuDeleteVisibleHandler,
                    AlwaysCheckStatus:=True))
...
#Region "Populate with data from the resources file"
        ...
        Private Sub PopulateResources(NewCategory As Category)
...
#Region "General UI"
        ...
        Public Overridable Function DsMsgBox(Message As String,
            ...
            Return DesignerMessageBox.Show(DirectCast(RootDesigner, IServiceProvider), Message, s_messageBoxCaption,
...
#Region "Integration with Visual Studio's Properties Window"
        ...
        Public Sub PropertyGridUpdate()
            If Not _propertyGridNeedUpdate Then
                ...
                NativeMethods.PostMessage(Handle, WmUserConstants.WM_UPDATE_PROPERTY_GRID, 0, 0)
            End If
        End Sub
            ...
            If Not _disablePropertyGridSelect Then
                Try
                    ...
                    Using New WaitCursor
                        ...
                        End If
                    End Using
                Catch ex As Exception When ReportWithoutCrash(ex, "Exception in property grid select", NameOf(ResourceEditorView))
                End Try
            End If
        End Sub
        ...
        Private Sub ResourceListView_SelectedIndexChanged(sender As Object, e As EventArgs) Handles _resourceListView.SelectedIndexChanged
            ...
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(ResourceListView_SelectedIndexChanged), NameOf(ResourceEditorView))
            End Try
        End Sub
        ...
        Private Sub StringTable_RowStateChanged(sender As Object, e As DataGridViewRowStateChangedEventArgs) _
            ...
            Try
                If e.StateChanged = DataGridViewElementStates.Selected Then
                    ...
                    RootDesigner.InvalidateFindLoop(ResourcesAddedOrRemoved:=False)
                End If
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(StringTable_RowStateChanged), NameOf(ResourceEditorView))
            End Try
        End Sub
        ...
        Private Sub StringTable_CellEnter(sender As Object, e As DataGridViewCellEventArgs) Handles StringTable.CellEnter
            Try
                PropertyGridUpdate()
                RootDesigner.InvalidateFindLoop(ResourcesAddedOrRemoved:=False)
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(StringTable_CellEnter), NameOf(ResourceEditorView))
            End Try
        End Sub
...
#Region "Adding/Removing resources"
                    ...
                    If AddToProject AndAlso Not CopyFileIfExists AndAlso Not AlwaysAddNew Then
                        ...
                        For i As Integer = 0 To NewResources.Length - 1
                            ...
                            Else
                                ...
                                If DsMsgBox(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Err_CantFindResourceFile_1Arg, Resource.AbsoluteLinkPathAndFileName) & VB.vbCrLf & VB.vbCrLf & My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Dlg_ContinueAnyway, MessageBoxButtons.YesNo, MessageBoxIcon.Error, ,
        ...
        Public Sub AddResources(NewResources As ICollection(Of Resource), CopyFileIfExists As Boolean, AddToProject As Boolean)
            ...
            Using New WaitCursor
                ...
                Try
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    If File.Exists(FinalPathAndFileName) Then
                                        ...
                                        If Not Path.GetFileNameWithoutExtension(FinalPathAndFileName).Equals(Path.GetFileNameWithoutExtension(Resource.AbsoluteLinkPathAndFileName),
                    ...
                    If ResourcesReadyToAdd.Count > 0 Then
                        Using Transaction As DesignerTransaction = RootDesigner.DesignerHost.CreateTransaction(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Undo_AddResources_1Arg, CStr(ResourcesReadyToAdd.Count)))
        ...
        Private Function CreateNewResourceFromFile(ResourceSourceFilePath As String) As Resource
            ...
            Dim Extension As String = Path.GetExtension(ResourceSourceFilePath)
                ...
                For Each Editor As ResourceTypeEditor In Category.AssociatedResourceTypeEditors
                    Try
                        ...
                        If Priority > 0 AndAlso Priority >= HighestPriority Then
                            ...
                            Exit For
                        End If
                    Catch ex As Exception When ReportWithoutCrash(ex, NameOf(GetResourceTypeEditorForFileExtension), NameOf(ResourceEditorView))
...
#Region "Drag-drop, Cut, Copy, Cancel edit, Delete and Paste Menu Commands"
        ...
        Private Function MenuGenericRemoveEnabledHandler(MenuCommand As DesignerMenuCommand) As Boolean
            Return RemoveButtonShouldBeEnabled()
        End Function

        Private Sub MenuGenericRemove(sender As Object, e As EventArgs)
        ...
        Private Sub ClearCells()
            ...
            Try
                ...
                Using Transaction As DesignerTransaction = RootDesigner.DesignerHost.CreateTransaction(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Undo_DeleteResourceCell, cells.Count))
        ...
        Private Sub MenuPaste(sender As Object, e As EventArgs)
            ...
            If DataFormatSupported(Data, DragDropEffects.Copy, ActualEffect, ActualFormat) Then
                ...
                Catch ex As Exception When ReportWithoutCrash(ex, NameOf(MenuPaste), NameOf(ResourceEditorView))
                    DsMsgBox(ex)
                End Try
            End If
        End Sub
...
#Region "Drag/drop"
        ...
        Private Shared Function DetermineCopyOrMove(CopyAllowed As Boolean, MoveAllowed As Boolean) As DragDropEffects
            ...
            If (ModifierKeys And Keys.Control) <> 0 AndAlso CopyAllowed Then
        ...
        Protected Overrides Sub OnDragDrop(e As DragEventArgs)
            ...
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(OnDragDrop), NameOf(ResourceEditorView))
                DsMsgBox(ex)
...
#Region "Drag/drop and paste dual implementation"
        ...
        Private Sub DragDropPaste(Data As IDataObject, AllowedEffects As DragDropEffects, ByRef ActualEffect As DragDropEffects)
            Dim ActualFormat As ResourceEditorDataFormats
        ...
        '''   the new file will copied with a unique name.  If False, the user will be asked whether the old file should be overwritten.</param>
        ''' <remarks>Caller must catch exceptions and display error message</remarks>
        Private Sub DragDropPasteFromWindowsExplorer(Data As IDataObject, CopyFileIfExists As Boolean)
        ...
        ''' <param name="Data">The data object from the source</param>
        ''' <remarks>Caller must catch exceptions and display error message</remarks>
        Private Sub DragDropPasteFromCsv(Data As IDataObject)
            ...
            If CurrentCategory.CategoryDisplay = Category.Display.StringTable AndAlso
                    StringTable.CurrentCell IsNot Nothing AndAlso
                    StringTable.CurrentCell.Selected AndAlso
                    StringTable.SelectedCells.Count = 1 AndAlso
                    Not StringTable.CurrentCell.ReadOnly AndAlso
        ...
        ''' <param name="Data">The data object from the source</param>
        ''' <remarks>Caller must catch exceptions and display error message</remarks>
        Private Sub DragDropPasteFromUnicodeText(Data As IDataObject)
        ...
        Private Shared Function GetTextFromIDataObject(Data As IDataObject, ClipboardFormat As String) As String
            ...
            ElseIf TypeOf RawData Is MemoryStream Then
                Dim StreamReader As New StreamReader(DirectCast(RawData, MemoryStream), Encoding.Default)
                Text = StreamReader.ReadToEnd()
                If Text.Length > 0 AndAlso Text(Text.Length - 1) = Chr(0) Then
        ...
        ''' <param name="ActualEffect">The actual effect which took place (copy or move)</param>
        ''' <remarks>Caller must catch exceptions and display error message</remarks>
        Private Sub DragDropPasteFromResourceEditor(Data As IDataObject, CopyFileIfExists As Boolean, ByRef ActualEffect As DragDropEffects)
        ...
        Private Shared Function GetFileListFromVsHDropHandle(HDropStream As Stream) As DraggedFileInfo()
            ...
            If HDropStream.Length <= Integer.MaxValue Then
                ...
                Dim hDrop As IntPtr = Marshal.AllocHGlobal(StreamLength)
                ...
                Try
                    ...
                    Dim Count As Integer = NativeMethods.DragQueryFile(hDropHandleRef.Handle, -1, Nothing, 0)
                    If Count > 0 Then
                        ReDim Files(Count - 1)

                        For i As Integer = 0 To Count - 1
                            Dim FileNameAndInfo As String = StrDup(FILEINFO_LENGTH, Chr(0))
                            Dim CharLen As Integer = NativeMethods.DragQueryFile(hDropHandleRef.Handle, i, FileNameAndInfo, FileNameAndInfo.Length)
                            If FileNameAndInfo.Length < CharLen Then
                                ' Alloc enough space and give another try...
                                FileNameAndInfo = StrDup(CharLen + 1, Chr(0))
                                CharLen = NativeMethods.DragQueryFile(hDropHandleRef.Handle, i, FileNameAndInfo, FileNameAndInfo.Length)
                            End If
                            If FileNameAndInfo.Length > CharLen Then
...
#Region "Play"
        ...
        Private Sub MenuPlay(sender As Object, e As EventArgs)
            ...
            Catch ex As Exception
                DsMsgBox(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Err_CantPlay_1Arg, ex.Message), MessageBoxButtons.OK, MessageBoxIcon.Error, , HelpIDs.Err_CantPlay)
            End Try
        End Sub
...
#Region "Open/Open With"
        ...
        ''' </remarks>
        Private Sub EditOrOpenWith(Resource As Resource, UseOpenWithDialog As Boolean)
            ...
            Dim ResourceFullPathTolerant As String = GetFullPathTolerant(ResourceFilePath)
            ...
            If Not File.Exists(ResourceFilePath) Then
                DsMsgBox(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Err_CantFindResourceFile_1Arg, ResourceFilePath), MessageBoxButtons.OK, MessageBoxIcon.Error, , HelpIDs.Err_CantFindResourceFile)
                Exit Sub
            End If
            ...
            Try
                ...
                Else
                    ...
                    OpenLogView = LOGVIEWID.LOGVIEWID_Primary
                End If
...
#Region "Import/Export commands"
                ...
                If SelectedResources IsNot Nothing AndAlso SelectedResources.Length > 0 Then
                    If CanImportOrExportResources(SelectedResources) Then
                        If SelectedResources.Length = 1 Then
                            ...
                            Dim Title As String = My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_DlgTitle_Export_1Arg, ResourceToExportFrom.Name)
                    ...
                    End If
                End If
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(MenuExport), NameOf(ResourceEditorView))
                DsMsgBox(ex)
            End Try
        End Sub
        ...
        Private Sub MenuImport(sender As Object, e As EventArgs)
            ...
            If SelectedResources IsNot Nothing AndAlso SelectedResources.Length = 1 Then
                Try
                    ...
                    RootDesigner.DesignerLoader.ManualCheckOut()
                Catch ex As Exception When ReportWithoutCrash(ex, NameOf(MenuImport), NameOf(ResourceEditorView))
                ...
                If CanImportOrExportResources(SelectedResources) Then
                    ...
                    Dim Title As String = My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_DlgTitle_Import_1Arg, ResourceToImportTo.Name)
        ...
        Public Shared Function GetSuggestedFileNameForResource(Resource As Resource) As String
            ...
            Else
                ...
                Catch ex As Exception When ReportWithoutCrash(ex, NameOf(GetSuggestedFileNameForResource), NameOf(ResourceEditorView))
        ...
        Public Function QueryUserToReplaceFiles(FileNames As IList) As Boolean
            ...
            ElseIf FileNames.Count = 1 Then
                Return DialogResult.Yes = DsMsgBox(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Dlg_ReplaceExistingFile, CStr(FileNames(0))), MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2)
            Else
        ...
        Private Sub ExportResources(Resources() As Resource, FolderPath As String, Optional SingleFileName As String = Nothing)
            ...
            Try
                ...
                If ExistingFiles.Count > 0 Then
                    ...
                    ' Try to checkout ...
                    SourceCodeControlManager.QueryEditableFiles(serviceProvider, ExistingFiles, False, False)
                End If
                ...
                For ResourceIndex As Integer = 0 To Resources.Length - 1
                    ...
                    If File.Exists(FileAndPathToExportTo) Then
                        ...
                        If Not SourceCodeControlManager.QueryEditableFiles(serviceProvider, ExistingFiles, False, True) Then
                            Continue For
                        End If
                ...
                Next
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(ExportResources), NameOf(ResourceEditorView))
                DsMsgBox(ex)
            End Try
        End Sub
        ...
        Private Function MenuEditLabelEnabledHandler(MenuCommand As DesignerMenuCommand) As Boolean
            If _currentCategory IsNot Nothing Then
                ...
                ElseIf CurrentCategory.CategoryDisplay = Category.Display.StringTable Then
                    Return Not ReadOnlyMode AndAlso
                        StringTable.CurrentCell IsNot Nothing AndAlso
                        Not StringTable.CurrentCell.ReadOnly AndAlso
...
#Region "Category buttons"
        ...
        Handles _
            ...
            _categoryFiles.ResourcesExistChanged,
            _categoryIcons.ResourcesExistChanged,
            _categoryImages.ResourcesExistChanged,
            _categoryOther.ResourcesExistChanged,
...
#Region "Add Existing menu item (add new resource from existing file)"
        ...
        Private Sub ButtonAdd_ExistingFile_Click(sender As Object, e As EventArgs)
            ...
            Catch ex As Exception When ReportWithoutCrash(ex, NameOf(ButtonAdd_ExistingFile_Click), NameOf(ResourceEditorView))
                DsMsgBox(ex)
            End Try
        End Sub
...
#Region "Add Button menus items (add new resources of a given type)"
        ...
        Public Function GetSaveLocationForNewProjectFile(TypeEditor As ResourceTypeEditor, FileNameOnly As String, ByRef UserCancel As Boolean) As String
        ...
        Private Sub QueryAddNewLinkedResource(TypeEditor As ResourceTypeEditor, ResourceFileExtension As String)
            Try
                ...
                Dim NewResourceFilePath As String = GetSaveLocationForNewProjectFile(
                    TypeEditor,
                    FileNameOnly,
                    UserCancel)
                If UserCancel Then
                ...
                Using New WaitCursor
                    ...
                    Catch ex As Exception When ReportWithoutCrash(ex, My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Err_CantCreateNewResource_2Args, NewResourceFilePath, ex.Message), NameOf(ResourceEditorView))
                        DsMsgBox(My.Resources.Microsoft_VisualStudio_Editors_Designer.GetString(My.Resources.Microsoft_VisualStudio_Editors_Designer.RSE_Err_CantCreateNewResource_2Args, NewResourceFilePath, ex.Message), MessageBoxButtons.OK, MessageBoxIcon.Error, , HelpIDs.Err_CantCreateNewResource)
                        Exit Sub
                    End Try
                    ...
                    If Project IsNot Nothing AndAlso
                    ...
                    Catch ex As Exception When ReportWithoutCrash(ex, NameOf(QueryAddNewLinkedResource), NameOf(ResourceEditorView))
                        DsMsgBox(ex)
                    End Try
                    ...
                    Catch ex As Exception When ReportWithoutCrash(ex, "Got an exception trying to open the new resource in a separate editor - ignoring and continuing", NameOf(ResourceEditorView))
...
#Region "Handling context menus"
        ...
        Private Sub ShowContextMenu(sender As Object, e As MouseEventArgs)
            If e.Button = System.Windows.Forms.MouseButtons.Right Then
                ...
                Catch ex As Exception When ReportWithoutCrash(ex, NameOf(ShowContextMenu), NameOf(ResourceEditorView))
                End Try
            Else
...
#Region "File dialog utilities"
        ...
        ''' <returns>The selected file/path, or Nothing if the user canceled.</returns>
        ''' <remarks>Caller is responsible for displaying any error messageboxes</remarks>
        Public Function ShowSaveFileDialog(ByRef UserCanceled As Boolean, Title As String, Filter As String,
                FilterIndex As Integer,
                Optional DefaultFileName As String = Nothing,
                Optional DefaultPath As String = Nothing, Optional OverwritePrompt As Boolean = False) As String

            UserCanceled = True

            Dim newFileName As String = GetNewFileNameViaBrowse(DirectCast(RootDesigner, IServiceProvider),
...
#Region "Temporary files and folders management"
        ...
        Private Shared Sub DeleteTemporaryFiles(FilesToDelete As IList(Of String))
            For Each FileName In FilesToDelete
                Try
                    ...
                    End If
                Catch ex As Exception When ReportWithoutCrash(ex, $"Unable to delete temporary file: {FileName}", NameOf(ResourceEditorView))
                End Try
            Next
        End Sub
        ...
        Private Shared Sub DeleteTemporaryFolders(FoldersToDelete As IList(Of String))
            For Each FolderName In FoldersToDelete
                Try
                    ...
                    End If
                Catch ex As Exception When ReportWithoutCrash(ex, $"Unable to delete temporary folder: {FolderName}", NameOf(ResourceEditorView))
                End Try
            Next
        End Sub
        ...
        Private Function GetTemporarySubfolder() As String
            ...
            Try
                ...
                Return NewFolder
            Catch ex As Exception When ReportWithoutCrash(ex, "Couldn't create subfolder under TEMP directory - using TEMP directory itself instead", NameOf(ResourceEditorView))
            End Try
        ...
        Private Function GetTemporaryFile(AutoDelete As AutoDelete, Optional ParentDirectory As String = "", Optional PreferredFileName As String = "") As String
            ...
            Select Case AutoDelete
                Case AutoDelete.DeleteOnClipboardFlush
        ...
        Public Function GetTypeResolutionService() As ITypeResolutionService Implements Resource.ITypeResolutionContextProvider.GetTypeResolutionService
            ...
            Try
                ...
                Else
                    ...
                    End If
                End If
            Catch ex As Exception When ReportWithoutCrash(ex, "Exception trying to retrieve project references", NameOf(ResourceEditorView))
                _typeResolutionServiceCache = Nothing
            End Try
            ...
            If Project IsNot Nothing Then
                Try
                    ...
                    If RootNamespaceProperty IsNot Nothing Then
                        ...
                        RootNamespace = DesignUtil.GenerateValidLanguageIndependentNamespace(RootNamespace)
                    End If
